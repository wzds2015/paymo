...
def main():
    ...
    batch_file = project_dir + '/paymo_input/batch_payment.csv'
    stream_file = project_dir + '/paymo_input/stream_payment.csv'
    outfile1 = project_dir + '/paymo_output/output1.txt'
    outfile2 = project_dir + '/paymo_output/output2.txt'
    outfile3 = project_dir + '/paymo_output/output3.txt'

    print "Building Degree1 Graph"
    degree1_graph = build_degree1_graph(read_input(batch_file))
    print "Working on feature1"
    feature1(stream_file, outfile1, degree1_graph)

    print "Building Degree2 Graph"
    degree2_graph = build_degree2_graph(degree1_graph)
    degree1_graph = dict_list2set(degree1_graph)
    print "Working on feature2"
    feature2(stream_file, outfile2, degree1_graph, degree2_graph)

    print "Working on feature3"
    feature3(stream_file, outfile3, degree1_graph, degree2_graph)

    end = time.time()
    print "Time elapsed: %s minutes" % ((end - start) / 60)
